...
public class QuestionController {
    ...
    @Tags({@Tag(name = "question-controller-admin"), @Tag(name = "question-controller")})
    @Operation(summary = "API get questions by user")
    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    @GetMapping(UrlConstant.Question.GET_QUESTIONS)
    public ResponseEntity<?> getQuestions(@RequestParam String userId, @Valid @ParameterObject PaginationFullRequestDto requestDTO) {
        return VsResponseUtil.success(questionService.getQuestionByUserId(userId, requestDTO));
    }
